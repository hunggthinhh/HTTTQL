    ...
    def _check_low_stock_alert(self):
        ...
        if self.current_qty < self.min_qty:
            ...
            if not existing_alert:
                ...
                # Gán người phụ trách quầy kệ hoặc người tạo nếu không có
                responsible = self.location_id.responsible_id or self.env.user
                
                self.env['bhx.stock.alert'].create({
                    'name': f'HẾT HÀNG KỆ: {self.product_id.name} ({self.location_id.name})' if alert_type == 'out_of_stock' else f'THIẾU KỆ: {self.product_id.name} ({self.location_id.name})',
                    ...
                    'note': f'Sản phẩm tại {self.location_id.name} (Dãy {self.location_id.aisle or "N/A"}) đang thiếu hàng.',
